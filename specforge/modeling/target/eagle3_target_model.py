...
from specforge.distributed import get_tp_group, get_tp_device_mesh
...
class Eagle3TargetModel(ABC):
    ...
    def set_aux_hidden_states_layers(self, aux_hidden_states_layers: Optional[List[int]] = None) -> None:
        """
...
class SGLangEagle3TargetModel(Eagle3TargetModel):
    ...
    def from_pretrained(cls,
        ...
        **kwargs
        ...
        wrap_eagle3_logits_processors_in_module(model_runner.model, return_full_logits=False)
    ...
    def extend(self, reqs, capture_aux_hidden_states: bool = True):
        ...
        if capture_aux_hidden_states:
            aux_hidden_states_list = torch.split(logits_output.aux_hidden_states, input_lens, dim=0)
        else:
            aux_hidden_states_list = None
    ...
    @torch.no_grad()
    def generate_eagle3_data(self, input_ids: torch.Tensor, attention_mask: torch.Tensor, loss_mask: torch.Tensor) -> Eagle3TargetOutput:
        """
        return:
        ...
        logits_list, aux_hidden_states_list = self.extend(reqs, capture_aux_hidden_states=True)
