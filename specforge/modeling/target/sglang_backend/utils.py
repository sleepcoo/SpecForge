...
def replaced_logits_processor_forward_for_eagle3(
    ...
    if (
        ...
        raise RuntimeError(f"The modified logits processor is not supported for this forward mode: {logits_metadata.forward_mode}")
...
class LogitsProcessorForEAGLE3(torch.nn.Module):
    ...
    def forward(
        ...
        ret = replaced_logits_processor_forward_for_eagle3(
            self.logits_processor,input_ids, hidden_states, lm_head, logits_metadata, aux_hidden_states
        )
        return ret
